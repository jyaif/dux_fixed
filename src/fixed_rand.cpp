...
inline int dux_clz(uint32_t __x) {
  return __builtin_clz(__x);
}
inline int dux_clz(uint64_t __x) {
...
class __independent_bits_engine {
 ...
 private:
  ...
  typedef
      typename std::conditional<sizeof(_Engine_result_type) <=
                                    sizeof(result_type),
                                result_type,
...
template <class _Engine, class _UIntType>
_UIntType __independent_bits_engine<_Engine, _UIntType>::__eval(
    std::true_type) {
...
dux::FInt FRandGenerator::RandFInt(dux::FInt min, dux::FInt max) {
  assert(max >= min);
  return dux::FInt::FromRawValue(
      UniformIntDistribution<int64_t>(rng_, min.raw_value_, max.raw_value_));
}
